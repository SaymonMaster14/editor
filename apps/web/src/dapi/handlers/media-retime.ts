...
export const mediaRetime: ToolHandler<"media_retime"> = async ({ path, fps, speed, reverse, freeze, ramp }, ctx) => {
  ...
  // The effective fps is hashed, not the raw option: an omitted fps and
  // an explicit fps equal to the asset rate are the same analysis.
  const effectiveFps = fps ?? asset.frameRate;
  const { result, cached } = await analyzeCached({
    kind: "media_retime",
    engine: "diffusion-temporal",
    engineVersion: "1",
    source: blob,
    params: { fps: effectiveFps, speed, reverse, freeze, ramp },
    ...
    run: async () => {
      ...
      try {
        ...
        let found;
        try {
          found = retime(frames, effectiveFps, {
